-- lua/autorun/server/sv_glide_base_vehicles_plates.lua
...
local function ApplyExternalConfig(ent)
    ...
    local config = externalConfigs[ent:GetClass()]
    ...
    if config and ent.IsGlideVehicle then
        ...
        -- Inject COPIES of the configurations into the entity: sharing the same
        -- table between instances would leak per-vehicle edits (color, skin...)
        -- to every other vehicle of the same class.
        ent.LicensePlateConfigs = table.Copy(base)

        if advanced then
            ent.LicensePlateAdvancedConfigs = table.Copy(advanced)
        end
...
hook.Add("OnEntityCreated", "GlideExternalPlates_Init", function(ent)
    -- Cheap synchronous early-out: only classes present in externalConfigs matter
    if not externalConfigs[ent:GetClass()] then return end
...
-- NOTE: the old "OnEntityCopyTable" hook was removed: that global hook does not
-- exist in GMod (the duplicator only calls the ENTITY method
-- OnEntityCopyTableFinish), so it never ran. Persistence for dupes/saves is
-- handled by the "glide_license_plate_data" entity modifier in
-- sh_glide_license_plates.lua, plus this file re-injecting configs on spawn.